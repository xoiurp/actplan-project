...
def extract_pendencias_debito(text):
    ...
    while i < len(lines):
        ...
        if (receita_match or simples_nac_match or no_code_item_match) and current_cnpj:
            ...
            # Para SIMPLES NAC., aceita se tiver dados reais
            if is_simples_nac:
                if has_real_period or has_real_due_date or has_financial_data:
                    ...
                    print(f"✅ SIMPLES NAC. extraído com dados reais: {debito_data}", file=sys.stdout)
                    i = j  # Continua da linha onde parou a coleta
                else:
                    print(f"❌ SIMPLES NAC. rejeitado - sem dados reais: {debito_data}", file=sys.stdout)
                    i += 1
