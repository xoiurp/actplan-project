...
def extract_pendencias_debito(text):
    ...
    while i < len(lines):
        ...
        if (receita_match or simples_nac_match or no_code_item_match) and current_cnpj:
            ...
            # Validação melhorada: aceita itens sem código se tiver pelo menos período e vencimento
            ...
            if has_basic_data and (debito_data.get("receita") or has_financial_data):
                ...
                print(f"❌ Falha na validação melhorada. Período: {debito_data.get('periodo_apuracao')}, Vencimento: {debito_data.get('vencimento')}, Receita: {debito_data.get('receita')}, Dados financeiros: {has_financial_data}", file=sys.stdout)
                i += 1
