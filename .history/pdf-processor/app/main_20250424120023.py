...
def extract_parcelamentos_siefpar(text):
    ...
    while i < len(lines):
        ...
        parcelamento_match = re.match(r"Parcelamento:\s*(.*)", line, re.IGNORECASE)
        if parcelamento_match and current_cnpj:
            parcelamento_data = {"cnpj": current_cnpj}
            parcelamento_data["parcelamento"] = parcelamento_match.group(1).strip()
            print(f"\nInício de registro SIEFPAR encontrado: '{line}'", file=sys.stdout)

            # Tenta ler as próximas linhas para Valor Suspenso e Modalidade
            valor_suspenso = 0.0
            modalidade = ""
            
            # Procura por Valor Suspenso nas próximas linhas (até 2 linhas de distância)
            for k in range(1, 3):
                if i + k < len(lines):
                    next_line = lines[i+k].strip()
                    valor_match = re.match(r"Valor Suspenso:\s*([\d.,]+)", next_line, re.IGNORECASE)
                    if valor_match:
                        valor_suspenso = parse_br_currency(valor_match.group(1))
                        print(f"Valor Suspenso encontrado: {valor_suspenso} na linha: '{next_line}'", file=sys.stdout)
                        break # Encontrou, pode parar de procurar valor

            # Procura por Modalidade nas próximas linhas (até 3 linhas de distância)
            for k in range(1, 4):
                 if i + k < len(lines):
                    next_line = lines[i+k].strip()
                    # Verifica se a linha não é outro campo chave como "Parcelamento:" ou "Valor Suspenso:"
                    if not re.match(r"(Parcelamento:|Valor Suspenso:)", next_line, re.IGNORECASE):
                         # Assume que a primeira linha não-chave após Parcelamento/Valor é a modalidade
                         modalidade_match = re.match(r"(Modalidade:)?\s*(.*)", next_line, re.IGNORECASE)
                         if modalidade_match and modalidade_match.group(2):
                             modalidade = modalidade_match.group(2).strip()
                             print(f"Modalidade encontrada: '{modalidade}' na linha: '{next_line}'", file=sys.stdout)
                             break # Encontrou, pode parar de procurar modalidade

            parcelamento_data["valor_suspenso"] = valor_suspenso
            ...
            # Avança o índice para depois da linha do parcelamento (as linhas de valor/modalidade já foram consideradas)
            i += 1 
