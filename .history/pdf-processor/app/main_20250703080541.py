...
def extract_pendencias_debito(text):
    ...
    while i < len(lines):
        ...
        simples_nac_match = re.match(r"^SIMPLES\s+NAC\.\s*$", line, re.IGNORECASE)
        
        # DETECÇÃO MELHORADA DE ITENS SEM CÓDIGO
        no_code_item_match = None
        if not receita_match and not simples_nac_match and current_cnpj:
            # Padrões mais abrangentes para detectar itens sem código
            no_code_patterns = [
                # Padrão 1: Linha que começa com período de apuração
                r"^(\d{2}/\d{2}/\d{4}|\d{2}/\d{4}|\d{1,2}(?:º|o|ª|\s)?\s*TRIM/\d{4})",
                # Padrão 2: Linha que começa com descrição seguida de período
                r"^([A-Z][A-Z\s]{2,})\s+(\d{2}/\d{2}/\d{4}|\d{2}/\d{4})",
                # Padrão 3: Linha que começa com valor monetário (formato brasileiro)
                r"^([\d.,]+)\s+",
                # Padrão 4: Linha que contém "SIMPLES" mas não exatamente "SIMPLES NAC."
                r"^(.*SIMPLES.*)",
                # Padrão 5: Linha que começa com texto descritivo comum em débitos
                r"^(CONTRIBUIÇÃO|IMPOSTO|TAXA|MULTA|JUROS|PIS|COFINS|CSLL|IRPJ)",
                # Padrão 6: Qualquer linha que não seja cabeçalho e tenha pelo menos 3 caracteres
                r"^([A-Z0-9][^:]{2,})"
            ]
            
            for pattern_idx, pattern in enumerate(no_code_patterns):
                no_code_item_match = re.match(pattern, line, re.IGNORECASE)
                if no_code_item_match:
                    print(f"🔍 Item sem código detectado (padrão {pattern_idx + 1}): '{line}'", file=sys.stdout)
                    break
            
            # Se ainda não encontrou, tenta uma detecção mais agressiva
            if not no_code_item_match and len(line) > 3 and not line.startswith("CNPJ:") and "Receita" not in line:
                # Verifica se a linha não é um cabeçalho conhecido
                header_keywords = ["PA/Exerc", "Vcto", "Vl. Original", "Sdo. Devedor", "Multa", "Juros", "Situação"]
                if not any(keyword in line for keyword in header_keywords):
                    no_code_item_match = True  # Marca como possível item sem código
                    print(f"🔍 Item sem código detectado (detecção agressiva): '{line}'", file=sys.stdout)
        
        if (receita_match or simples_nac_match or no_code_item_match) and current_cnpj:
            ...
            if receita_match:
                ...
                # SIMPLES NAC. em linha separada - dados nas próximas linhas
                print(f"🎯 SIMPLES NAC. detectado em linha separada: '{line}'", file=sys.stdout)
            ...
            # Validação especial para SIMPLES NAC. - mais flexível
            ...
            # Para SIMPLES NAC., aceita mesmo com dados mínimos
            if is_simples_nac:
                # Para SIMPLES NAC., aceita se tiver pelo menos período OU vencimento OU dados financeiros
                has_minimal_data = (debito_data.get("periodo_apuracao") or 
                                  debito_data.get("vencimento") or 
                                  has_financial_data)
                
                if has_minimal_data:
                    # Preenche campos vazios com valores padrão para SIMPLES NAC.
                    ...
                    print(f"✅ SIMPLES NAC. extraído com validação flexível: {debito_data}", file=sys.stdout)
                    i = j  # Continua da linha onde parou a coleta
                else:
                    print(f"❌ SIMPLES NAC. sem dados mínimos: {debito_data}", file=sys.stdout)
                    i += 1
            elif has_basic_data and (debito_data.get("receita") or has_financial_data):
                # Validação normal para outros tipos
                # Se não tem receita mas tem dados financeiros, gera um código baseado no período
                if not debito_data.get("receita") or debito_data.get("receita") == "SEM CÓDIGO":
                    if has_financial_data:
                        periodo_code = debito_data.get("periodo_apuracao", "").replace("/", "-")
                        debito_data["receita"] = f"ITEM-{periodo_code}" if periodo_code else "ITEM-SEM-CODIGO"
                        print(f"🔧 Código gerado automaticamente: '{debito_data['receita']}'", file=sys.stdout)
                    else:
                        debito_data["receita"] = "ITEM-SEM-CODIGO"
                        print(f"🔧 Item sem código e sem valores financeiros, usando código padrão", file=sys.stdout)
                
                result.append(debito_data)
                print(f"✅ Item extraído com sucesso (validação melhorada): {debito_data}", file=sys.stdout)
                i = j  # Continua da linha onde parou a coleta
            else:
                print(f"❌ Falha na validação. Período: {debito_data.get('periodo_apuracao')}, Vencimento: {debito_data.get('vencimento')}, Receita: {debito_data.get('receita')}, Dados financeiros: {has_financial_data}", file=sys.stdout)
                i += 1
