...
def extract_pendencias_debito(text):
    ...
    pattern = r"(?:Pendência|Pendencia|PENDÊNCIA|PENDENCIA)[\s-]*(?:Débito|Debito|DÉBITO|DEBITO)[\s-]*(?:\(SIEF\)|\(sief\))(.*?)(?=\n\s*(?:Pendência|Pendencia|PENDÊNCIA|PENDENCIA|Parcelamento|Processo|Inscrição|Débito com Exigibilidade|Final do Relatório)|__________________________|\Z)"
    matches = re.finditer(pattern, text, re.DOTALL | re.IGNORECASE | re.UNICODE)
