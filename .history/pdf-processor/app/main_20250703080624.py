...
def extract_pendencias_debito(text):
    ...
    while i < len(lines):
        ...
        if (receita_match or simples_nac_match or no_code_item_match) and current_cnpj:
            ...
            # VALIDAÇÃO ULTRA FLEXÍVEL - aceita qualquer item com dados mínimos
            ...
            # Critério ultra flexível: aceita se tiver QUALQUER dado útil
            has_any_useful_data = (
                debito_data.get("periodo_apuracao") or 
                debito_data.get("vencimento") or 
                has_financial_data or
                debito_data.get("situacao")
            )
            
            # Para SIMPLES NAC. ou itens sem código, usa validação ultra flexível
            if is_simples_nac or is_sem_codigo:
                if has_any_useful_data:
                    ...
                    if is_sem_codigo:
                        if debito_data.get("periodo_apuracao") and debito_data["periodo_apuracao"] != "N/A":
                            periodo_code = debito_data["periodo_apuracao"].replace("/", "-")
                            debito_data["receita"] = f"ITEM-{periodo_code}"
                        else:
                            debito_data["receita"] = f"ITEM-{len(result)+1:03d}"
                        print(f"🔧 Código gerado para item sem código: '{debito_data['receita']}'", file=sys.stdout)
                    
                    result.append(debito_data)
                    print(f"✅ Item extraído com validação ultra flexível: {debito_data}", file=sys.stdout)
                    i = j  # Continua da linha onde parou a coleta
                else:
                    print(f"❌ Item sem dados úteis: {debito_data}", file=sys.stdout)
                    i += 1
                ...
                i = j  # Continua da linha onde parou a coleta
            elif has_any_useful_data:
                # Fallback: aceita qualquer item com dados úteis, mesmo sem código completo
                if not debito_data.get("receita"):
                    if debito_data.get("periodo_apuracao"):
                        periodo_code = debito_data["periodo_apuracao"].replace("/", "-")
                        debito_data["receita"] = f"ITEM-{periodo_code}"
                    else:
                        debito_data["receita"] = f"ITEM-{len(result)+1:03d}"
                    print(f"🔧 Código gerado para item genérico: '{debito_data['receita']}'", file=sys.stdout)
                ...
                result.append(debito_data)
                print(f"✅ Item genérico extraído com fallback: {debito_data}", file=sys.stdout)
                i = j  # Continua da linha onde parou a coleta
            else:
                print(f"❌ Item rejeitado - sem dados úteis: {debito_data}", file=sys.stdout)
                i += 1
