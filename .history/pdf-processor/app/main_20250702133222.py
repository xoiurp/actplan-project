...
def extract_pendencias_debito(text):
    ...
    while i < len(lines):
        ...
        # Verifica se a linha começa com um código de receita (XXXX-XX - ...) ou "SIMPLES NAC."
        ...
        if receita_match and current_cnpj:
            ...
            if "SIMPLES NAC" in receita_text.upper():
                 # Para o Simples Nacional, a linha inteira é a descrição.
                 # Os valores estão na mesma linha, então precisamos de uma lógica diferente.
                 parts = line.split()
                 if len(parts) >= 8:
                      debito_data = {
                           "cnpj": current_cnpj,
                           "receita": " ".join(parts[0:2]),
                           "periodo_apuracao": format_periodo(parts[2]),
                           "vencimento": format_date(parts[3]),
                           "valor_original": parse_br_currency(parts[4]),
                           "saldo_devedor": parse_br_currency(parts[5]),
                           "multa": parse_br_currency(parts[6]),
                           "juros": parse_br_currency(parts[7]),
                           "saldo_devedor_consolidado": parse_br_currency(parts[8]) if len(parts) > 8 else 0.0,
                           "situacao": parts[9] if len(parts) > 9 else "DEVEDOR"
                      }
                      result.append(debito_data)
                      print(f"✅ Item SIMPLES NAC. extraído (linha única): {debito_data}", file=sys.stdout)
                      i += 1
                      continue # Pula para a próxima linha do loop principal
                 else:
                      print(f"❌ Linha SIMPLES NAC. com formato inesperado, pulando: '{line}'", file=sys.stdout)
                      i += 1
                      continue
            else:
