...
# Função para extrair dados do DARF
def extract_darf_data(text):
    result = []
    lines = text.split('\n')
    in_composition_section = False
    
    print("\n--- Iniciando extração de dados do DARF ---", file=sys.stdout)
    
    # Procura pela seção "Composição do Documento de Arrecadação"
    ...
    while i < len(lines):
        ...
        # Encontra o início da seção de composição
        if not in_composition_section and re.search(start_pattern, line, re.IGNORECASE):
            in_composition_section = True
            print(f"Seção de composição encontrada na linha {i+1}: '{line}'", file=sys.stdout)
            
            # Debug: mostra as próximas 10 linhas para entender o formato
            print("--- DEBUG: Próximas 10 linhas após encontrar a seção ---", file=sys.stdout)
            for debug_idx in range(i+1, min(i+11, len(lines))):
                debug_line = lines[debug_idx].strip()
                print(f"Linha {debug_idx+1}: '{debug_line}'", file=sys.stdout)
            print("--- FIM DEBUG ---", file=sys.stdout)
        ...
        # Para se chegar ao fim do documento ou nova seção
        if not line or line.startswith("Total do Documento") or line.startswith("VENCIMENTO"):
            break
            
        # Log da linha atual para debug
        if line:  # Só loga linhas não vazias
            print(f"Processando linha {i+1}: '{line}'", file=sys.stdout)
        
        # Tenta extrair dados de um item DARF
        # Formato esperado: CÓDIGO DENOMINAÇÃO ... PA XX/XXXX Vencimento XX/XX/XXXX VALORES
        darf_item_match = re.match(r"^(\d{4})\s+(.+)", line)
        if darf_item_match:
            codigo = darf_item_match.group(1)
            denominacao_e_resto = darf_item_match.group(2).strip()
            
            print(f"\nItem DARF encontrado - Código: {codigo}", file=sys.stdout)
            print(f"Linha completa: '{line}'", file=sys.stdout)
            
            # Extrai o período de apuração (PA XX/XXXX)
            periodo_match = re.search(r"PA\s+(\d{2}/\d{4})", denominacao_e_resto)
            periodo = periodo_match.group(1) if periodo_match else ""
            
            # Extrai a data de vencimento
            vencimento_match = re.search(r"Vencimento\s+(\d{2}/\d{2}/\d{4})", denominacao_e_resto)
            vencimento = vencimento_match.group(1) if vencimento_match else ""
            
            # Extrai a denominação (tudo antes de "PA")
            if periodo_match:
                denominacao = denominacao_e_resto[:periodo_match.start()].strip()
            else:
                denominacao = denominacao_e_resto.strip()
                
            # Remove "Vencimento..." do final da denominação se estiver lá
            if vencimento_match:
                denominacao = denominacao[:denominacao.find("Vencimento")].strip() if "Vencimento" in denominacao else denominacao
                
            print(f"Denominação: '{denominacao}'", file=sys.stdout)
            print(f"Período: '{periodo}'", file=sys.stdout)
            print(f"Vencimento: '{vencimento}'", file=sys.stdout)
            
            # Procura pelos valores nas próximas linhas
            valores_encontrados = []
            j = i + 1
            max_search_lines = 5  # Limita a busca a 5 linhas
            
            while j < len(lines) and len(valores_encontrados) < 4 and (j - i) <= max_search_lines:
                valor_line = lines[j].strip()
                
                # Para se encontrar outro código DARF
                if re.match(r"^\d{4}\s+", valor_line):
                    break
                    
                # Para se for linha vazia ou fim
                if not valor_line or valor_line.startswith("Total do Documento"):
                    break
                
                # Tenta extrair valores monetários da linha
                # Procura por padrões como: 43.218,90  8.643,78  4.991,78  56.854,46
                valores_na_linha = re.findall(r'([\d.,]+)', valor_line)
                
                for valor_str in valores_na_linha:
                    # Filtra apenas valores que parecem monetários (com vírgula ou múltiplos dígitos)
                    if ',' in valor_str or len(valor_str.replace('.', '')) > 3:
                        try:
                            valor_float = parse_br_currency(valor_str)
                            if valor_float > 0:
                                valores_encontrados.append(valor_float)
                                print(f"Valor encontrado: '{valor_str}' -> {valor_float}", file=sys.stdout)
                        except:
                            continue
                
                j += 1
            
            # Atribui os valores encontrados (Principal, Multa, Juros, Total)
            principal = valores_encontrados[0] if len(valores_encontrados) > 0 else 0.0
            multa = valores_encontrados[1] if len(valores_encontrados) > 1 else 0.0
            juros = valores_encontrados[2] if len(valores_encontrados) > 2 else 0.0
            total = valores_encontrados[3] if len(valores_encontrados) > 3 else principal + multa + juros
            ...
            # Continua da onde parou a busca por valores
            i = j - 1
