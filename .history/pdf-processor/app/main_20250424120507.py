    ...
    while i < len(lines):
        ...
        # Tenta encontrar um registro de parcelamento
            ...
            if parcelamento_valor_match and current_cnpj:
                ...
                # Procura pela Modalidade na próxima linha não vazia
                modalidade = ""
                k = i + 1
                while k < len(lines):
                    next_line = lines[k].strip()
                    if next_line: # Encontrou a próxima linha não vazia
                         # Assume que é a modalidade (pode ou não ter "Modalidade:")
                         ...
                         break # Para após verificar a primeira linha não vazia
                    k += 1 # Pula linhas vazias intermediárias se houver
                ...
                # Avança o índice para depois da linha da modalidade (ou da linha do parcelamento se não achou modalidade)
                i = k + 1 if modalidade else i + 1 
            
            else:
                # Se não encontrou o padrão Parcelamento/Valor, ignora a linha
