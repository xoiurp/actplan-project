...
# Função específica para extrair "Pendência - Débito (SIEF)" - Lógica v3 (Multilinha)
def extract_pendencias_debito(text):
    result = []
    # Identificar a seção de pendências - Tentativa com lookahead mais específico
    # Captura tudo após "Pendência - Débito (SIEF)" até encontrar outra linha começando com "Pendência", "Parcelamento", "Débito com Exigibilidade", "Inscrição", "Processo", "Final do Relatório" ou uma linha de underscores longos.
    pattern = r"(?:Pendência|Pendencia|PENDÊNCIA|PENDENCIA)[\s-]*(?:Débito|Debito|DÉBITO|DEBITO)[\s-]*(?:\(SIEF\)|\(sief\))\s*\n(.*?)(?=\n(?:Pendência|Pendencia|Parcelamento|Processo|Inscrição|Débito\s+com\s+Exigibilidade|Final\s+do\s+Relatório)|^\s*_{10,}\s*$|\Z)"
    matches = re.finditer(pattern, text, re.DOTALL | re.IGNORECASE | re.UNICODE | re.MULTILINE)
    ...
    for match_index, match in enumerate(matches):
        ...
        print(f"\n--- Processando Seção Débito SIEF #{match_index + 1} ---", file=sys.stdout)
        print(f"Linhas encontradas na seção: {len(lines)}", file=sys.stdout)
        ...
        while i < len(lines):
            ...
            # Ignora linhas de cabeçalho explícitas
            header_parts = ["Receita", "PA/Exerc", "Vcto", "Original", "Devedor", "Multa", "Juros", "Cons", "Situação"]
            if any(part in line for part in header_parts) and len(line.split()) > 3:
            ...
            if receita_match and current_cnpj:
                ...
                expected_fields = [
                    "periodo_apuracao", "vencimento", "valor_original",
                    "saldo_devedor", "multa", "juros",
                    "saldo_devedor_consolidado", "situacao"
                ]
                
                data_lines = lines[i+1 : i+1+len(expected_fields)] # Pega as próximas linhas
                print(f"Tentando ler {len(expected_fields)} campos nas próximas {len(data_lines)} linhas.", file=sys.stdout)

                if len(data_lines) == len(expected_fields):
                    try:
                        ...
                        if debito_data["receita"] and debito_data["periodo_apuracao"] and debito_data["vencimento"]:
                            ...
                            i += 1 + len(expected_fields) # Pula a linha da receita e as linhas de dados
                        else:
                    ...
                    print(f"Número incorreto de linhas ({len(data_lines)}) encontradas após a receita '{debito_data['receita']}'. Esperado {len(expected_fields)}.", file=sys.stdout)
                    i += 1 # Pula apenas a linha da receita
            else:
                # Se a linha não é CNPJ, cabeçalho ou início de receita, apenas pula
                print(f"Linha ignorada (não reconhecida): '{line}'", file=sys.stdout)
                i += 1
