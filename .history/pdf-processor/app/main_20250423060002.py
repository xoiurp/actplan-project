...
async def process_situacao_fiscal(file: UploadFile = File(...)):
    ...
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_file:
            ...
            # Configurar idioma para inglês (padrão) para evitar problemas
            # EasyOCR tem problemas com idiomas não suportados
            try:
                # Usar inglês como idioma padrão para evitar problemas
                pipeline_options.ocr_options.lang = ["en"] # Inglês
                logger.info("Usando idioma inglês para OCR")
            except Exception as lang_error:
                logger.warning(f"Não foi possível configurar idioma: {lang_error}")
