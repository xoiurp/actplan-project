...
def extract_pendencias_debito(text):
    ...
    while i < len(lines):
        ...
        if (receita_match or simples_nac_match or simples_nac_only_match or no_code_item_match) and current_cnpj:
            ...
            # Para SIMPLES NAC., usa validação mais flexível
            if is_simples_nac:
                # Para SIMPLES NAC., aceita se tiver QUALQUER dado útil ou se for reconhecido como Simples Nacional
                has_any_data = (has_real_period or has_real_due_date or has_financial_data or 
                               debito_data.get("situacao") or 
                               debito_data.get("receita") == "SIMPLES NAC.")
                ...
                if has_any_data:
                    ...
                    result.append(debito_data)
                    print(f"✅ SIMPLES NAC. extraído: {debito_data}", file=sys.stdout)
                    i = j  # Continua da linha onde parou a coleta
                else:
                    print(f"❌ SIMPLES NAC. rejeitado - sem dados úteis: {debito_data}", file=sys.stdout)
                    i += 1
