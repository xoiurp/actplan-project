...
function extractCertificateInfo(cert: any) {
  ...
  // Mapeamento de OIDs para campos
  const oidMap = {
    '2.16.76.1.3.3': 'cnpj',
    '2.16.76.1.3.1': 'razao_social',
    '2.16.76.1.3.2': 'nome_fantasia',
    '2.16.76.1.3.5': 'endereco',
    '2.16.76.1.3.6': 'cep',
    '2.16.76.1.3.7': 'cidade',
    '2.16.76.1.3.8': 'estado'
  };
  ...
  cert.subject.attributes.forEach((attr: any) => {
    ...
    // Verificar se o OID está no mapeamento
    const oid = attr.type;
    if (oidMap[oid]) {
      info[oidMap[oid]] = attr.value;
      console.log(`Found ${oidMap[oid]}:`, attr.value);
    }
  });
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        ...
        subject: cert.subject.attributes.map(attr => `${attr.type}=${attr.value}`).join(', '),
        issuer: cert.issuer.attributes.map(attr => `${attr.type}=${attr.value}`).join(', '),
