...
export default function EditCustomer() {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (certificateFile && formData.senha_certificado) {
      ...
      // If no new file, but requested to remove existing, clear dates
      validatedExpiryDate = null;
    }
    ...
    if (removeExistingCertificate) {
      customerDataToUpdate.certificado = null;
      customerDataToUpdate.certificado_validade = null;
      customerDataToUpdate.senha_certificado = undefined; // Don't send password if removing
    } else if (certificateFile) {
      // If a new certificate is being uploaded, its details will be set by `uploadCertificate`.
      // We only pass `certificado_validade` if validated, and `senha_certificado` for the validation step (already in formData).
      customerDataToUpdate.certificado_validade = validatedExpiryDate;
      // `certificado` object itself is not set here, `uploadCertificate` handles that post-customer-update.
    } else if (customer?.certificado) {
      // No new file, not removing existing: keep existing certificate details (already in formData from useEffect)
    ...
    <>
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6 bg-white rounded-lg">
        <CustomerForm
          ...
          isEdit={true}
          customerId={id}
          existingCertificateUrl={removeExistingCertificate ? null : customer.certificado?.url}
          existingCertificateName={removeExistingCertificate ? null : customer.certificado?.nome}
          certificateExpiryDate={removeExistingCertificate ? null : certificateExpiryDate}
          onRemoveCertificate={handleRemoveCertificate}
          onCertificateUpload={() => {
            refetchCustomer(); // Refetch customer data to show new certificate info
          }}
        />
      </div>
    </>
