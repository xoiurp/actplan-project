  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    if (certificateFile && formData.senha_certificado) {
      ...
      try {
        ...
        // Store the expiration date if valid
        if (validationResult.expirationDate) {
            ...
            setCertificateExpiryDate(formattedExpiry); 
        } else {
            setCertificateExpiryDate(null); // Clear if no date returned
        }
    ...
    // Include the formatted expiry date when mutating, ensuring undefined instead of null
    createMutation.mutate({
      ...formData,
      certificado_validade: certificateExpiryDate ?? undefined, // Pass undefined if null
      updated_at: null, // Add updated_at here as expected by createCustomer
    });
  }, [formData, certificateFile, certificateExpiryDate, createMutation]); // Add certificateExpiryDate to dependencies
