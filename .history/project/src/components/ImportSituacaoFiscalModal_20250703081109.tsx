...
export function ImportSituacaoFiscalModal({ 
  ...
  const handleConfirm = () => {
    ...
    try {
      ...
      // Validação adicional para itens sem código
      const validItems = orderItems.filter(item => {
        const hasBasicData = item.start_period || item.due_date || item.original_value > 0;
        if (!hasBasicData) {
          console.warn('Item sem dados básicos ignorado:', item);
          return false;
        }
        return true;
      });
