...
// Endpoint para o microserviço de extração via proxy Nginx
// Endpoint para o microserviço de extração via proxy Nginx (lido do objeto global)
const EXTRACTION_API_ENDPOINT = window._env_?.VITE_EXTRACTION_API_URL || '/api/extraction/extract/situacao-fiscal';
...
export function ImportSituacaoFiscalModal({ 
  ...
  const handleProcess = async () => {
    ...
    console.log('Attempting to fetch from:', EXTRACTION_API_ENDPOINT); // Log para depuração

    try {
      const formData = new FormData();
      formData.append('file', selectedFile);

      const url = EXTRACTION_API_ENDPOINT;
      console.log('Fetching to URL:', url); // Log da URL completa
