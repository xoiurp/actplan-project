...
export function ImportSituacaoFiscalModal({ 
  ...
  const handleConfirm = () => {
    ...
    try {
      ...
      // Validação rigorosa para filtrar itens vazios
      const validItems = orderItems.filter(item => {
        ...
        const hasRealValues = item.original_value > 0 || item.balance_due > 0;
        const hasRealCode = item.code && item.code !== "ITEM-N/A" && !item.code.startsWith("ITEM-00");
        
        // Item deve ter pelo menos período real OU vencimento real OU valores reais
        const hasMinimalRealData = hasRealPeriod || hasRealDueDate || hasRealValues;
        
        if (!hasMinimalRealData) {
          console.warn('Item sem dados reais ignorado:', item);
          return false;
        }
      ...
      if (validItems.length < orderItems.length) {
        toast(`${orderItems.length - validItems.length} itens sem dados básicos foram ignorados`, {
          icon: '⚠️',
          duration: 4000,
        });
      }
                ...
                {extractedData.pendenciasDebito.length > 0 && (
                  <div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">{extractedData.pendenciasDebito.map((item, index) => (<tr key={index}><td className="px-3 py-2 text-sm">{item.receita}</td><td className="px-3 py-2 text-sm">{item.periodo_apuracao}</td><td className="px-3 py-2 text-sm">{item.vencimento}</td><td className="px-3 py-2 text-sm text-right">{formatCurrency(item.valor_original)}</td><td className="px-3 py-2 text-sm text-right">{formatCurrency(item.saldo_devedor)}</td><td className="px-3 py-2 text-sm">{item.situacao}</td></tr>))}</tbody>
                    </table>
                  </div>
                )}
