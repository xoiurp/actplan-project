...
export function ImportSituacaoFiscalModal({ 
  ...
  const handleConfirm = () => {
    ...
    try {
      ...
      // Validação muito mais simples - aceita praticamente todos os itens convertidos
      const validItems = orderItems.filter(item => {
        // Critérios mínimos básicos - deve ter pelo menos um ID e um tipo
        const hasBasicStructure = item.id && item.tax_type;
        
        if (!hasBasicStructure) {
          console.warn('❌ Item sem estrutura básica ignorado:', item);
          return false;
        }
        
        // Log de cada item que está sendo validado
        console.log(`✅ Item aceito: ${item.tax_type} - ${item.code || 'sem código'}`, item);
        return true;
      });
      
      console.log(`🎯 Validação concluída: ${validItems.length} de ${orderItems.length} itens aceitos`);
      
      // Log detalhado dos itens rejeitados se houver
      if (validItems.length < orderItems.length) {
        const rejectedItems = orderItems.filter(item => !validItems.includes(item));
        console.warn('❌ Itens rejeitados:', rejectedItems);
      }
      ...
      if (validItems.length < orderItems.length) {
        ...
        toast(`${ignoredCount} itens sem estrutura básica foram ignorados`, {
          icon: '⚠️',
          duration: 4000,
        });
      }
