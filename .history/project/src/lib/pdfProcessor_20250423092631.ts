  ...
  try {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      if (text.includes('Parcelamento com Exigibilidade Suspensa (SIPADE)')) {
        ...
        const regex = /Processo\s+Receita\s+Situacao\s*([\s\S]*?)(?=\n\s*\n|$)/g;
        const matches = text.matchAll(regex);
        for (const match of matches) {
          const lines = match[1].split('\n').filter(line => line.trim());
          console.log(`Encontradas ${lines.length} linhas de parcelamentos SIPADE`);
          lines.forEach(line => {
            ...
            if (parts.length >= 3) {
              ...
              console.log('Parcelamento SIPADE adicionado:', parts[0]);
            }
          });
        }
      }
      ...
      if (text.includes('Pendência - Débito (SIEF)')) {
        ...
        currentSection = 'SIEF';
        const regex = /(\d{2}\/\d{4})\s+(\d{2}\/\d{2}\/\d{4})\s+(\d+[.,]\d+)\s+(\d+[.,]\d+)\s+(\d+[.,]\d+)\s+(\d+[.,]\d+)\s+(\d+[.,]\d+)\s+(\w+)(?:\s+CNO:\s+(\d+(?:\.\d+)*\/\d+))?/g;
        const matches = text.matchAll(regex);
        let count = 0;
        for (const match of matches) {
          result.pendenciasDebito.push({
            ...
            vencimento: match[2],
            valor_original: parseFloat(match[3].replace(',', '.')),
            saldo_devedor: parseFloat(match[4].replace(',', '.')),
            multa: parseFloat(match[5].replace(',', '.')),
            juros: parseFloat(match[6].replace(',', '.')),
            saldo_devedor_consolidado: parseFloat(match[7].replace(',', '.')),
            situacao: match[8],
            cno: match[9] || undefined
          });
          count++;
      ...
      if (text.includes('Processo Fiscal com Exigibilidade Suspensa (SIEF)')) {
        ...
        const regex = /(\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2})\s+([\w-]+)\s+(.*)/g;
        const matches = text.matchAll(regex);
        for (const match of matches) {
      ...
      if (text.includes('Parcelamento com Exigibilidade Suspensa (SIEFPAR)')) {
        ...
        const regex = /Parcelamento:\s*(\d+)\s*Valor Suspenso:\s*([\d,.]+)\s*(.*)/g;
        const matches = text.matchAll(regex);
        for (const match of matches) {
      ...
      if (text.includes('Débito com Exigibilidade Suspensa (SICOB)')) {
        ...
        const regex = /Parcelamento:\s*(\d+-\d+)\s*Situacao:\s*(\d+\s*-\s*[^]*?)\s*([^]*?)(?=\n|$)/g;
        const matches = text.matchAll(regex);
        for (const match of matches) {
      ...
      if (text.includes('Pendência - Inscrição (SIDA)')) {
        currentSection = 'SIDA';
        const regex = /(\d{2}\.\d{2}\.\d{6}-\d{2})\s+(\d{4}-[\w\s]+)\s+(\d{2}\/\d{2}\/\d{4})\s+(\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2})\s+([\w\s]+)\s+(\w+\s+\w+\s+\w+)/g;
        const matches = text.matchAll(regex);
        for (const match of matches) {
