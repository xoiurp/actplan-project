    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      if (text.includes('Pendência - Débito (SIEF)') || text.includes('Pendência - Débito ( SIEF )')) {
        ...
        if (tableMatch) {
          ...
          const regex = /(\d{4}-\d{2}\s*-\s*[A-Z]+(?:\s*-\s*[A-Z]+)?|\d{4}-\d{2}\s*-\s*[A-Z]+(?:\s+-\s+[A-Z]+)?)\s+(\d{2}\/\d{4})\s+(\d{2}\/\d{2}\/\d{4})\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\w+)(?:\s+CNO:\s*(\d+(?:\.\d+)*\/\d+))?/g;
