...
export async function processSituacaoFiscalPDF(file: File): Promise<SituacaoFiscalData> {
  try {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      // Normaliza os espaços extras e quebras de linha
      ...
      if (text.includes('Parcelamento com Exigibilidade Suspensa (SIPADE)')) {
        ...
        const regex = /(\d{5}\.\d{3}\.\d{3}\/\d{4}-\d{2})\s+(\d{4}-[A-Z]+)\s+(ATIVO|SUSPENSO|CANCELADO)/g;
      ...
      if (text.includes('Pendência - Débito (SIEF)')) {
        ...
        if (tableMatch) {
          ...
          const regex = /(\d{4}-\d{2}\s*-\s*[A-Z]+)\s+(\d{2}\/\d{4})\s+(\d{2}\/\d{2}\/\d{4})\s+(\d+[.,]\d+)\s+(\d+[.,]\d+)\s+(\d+[.,]\d+)\s+(\d+[.,]\d+)\s+(\d+[.,]\d+)\s+(\w+)(?:\s+CNO:\s*(\d+(?:\.\d+)*\/\d+))?/g;
          const matches = text.matchAll(regex);
          let count = 0;
          for (const match of matches) {
