...
export async function createOrder(order: any) {
  ...
  if (order.documentos) {
    ...
    // Remove a propriedade 'file' antes de salvar no banco
    const cleanDocuments = (docs: Order['documentos']) => {
        if (!docs) return docs;
        const cleaned: Partial<Order['documentos']> = {};
        for (const key in docs) {
            const typedKey = key as keyof Order['documentos'];
            const value = docs[typedKey];

            if (Array.isArray(value)) {
                cleaned[typedKey] = value.map((item) => {
                    if (!item) return item;
                ...
                cleaned[typedKey] = rest;
            } else {
                (cleaned as any)[typedKey] = value;
            }
        }
        return cleaned;
    };

    const finalDocumentos = cleanDocuments(documentos);
