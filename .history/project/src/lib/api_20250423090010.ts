...
import { getSupabaseClient } from './supabase'; // Importar a função
...
export async function getPaymentPlans(): Promise<PaymentPlan[]> {
  const supabaseClient = getSupabaseClient(); // Obter a instância do cliente
...
export async function createOrder(order: any) {
  ...
  if (order.documentos) {
    ...
    const supabaseClient = getSupabaseClient(); // Get client instance again if needed
    const { error: updateError } = await supabaseClient
      .from('orders')
      .update({ documentos })
      .eq('id', data.id)
...
export async function validateCertificateApi(pfxFile: File, password: string): Promise<{ isValid: boolean; expirationDate?: string; error?: string }> {
  try {
    ...
    const supabaseClient = getSupabaseClient(); // Obter a instância do cliente
    const { data, error } = await supabaseClient.functions.invoke('validate-certificate', {
