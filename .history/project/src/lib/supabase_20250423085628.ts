...
export function getSupabaseClient() {
  if (!supabaseInstance) {
    ...
    // Explicitly type the client with the correct schema generic
    supabaseInstance = createClient<'public'>(url, key, options);
  }
  // Ensure the return type matches the instance type
  return supabaseInstance as ReturnType<typeof createClient<'public'>>;
}
