  ...
  try {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      const text = textContent.items
        .map((item: any) => {
          const currentY = item.transform[5]; // Posição Y do item
          let result = item.str;
          
          // Se mudou a linha (Y diferente), adiciona quebra de linha
        ...
        .join(' ')
        .replace(/\s{3,}/g, '\n') // Converte 3 ou mais espaços em quebra de linha
      ...
      // Extrair CNPJ principal (mais flexível)
      ...
      if (text.includes('Pendência - Débito (SIEF)') || text.includes('Pendência - Débito ( SIEF )')) {
        ...
        // Extrai a tabela de débitos (mais flexível com espaços)
        const tableMatch = text.match(/Pendência\s*-\s*Débito\s*\(\s*SIEF\s*\)(.*?)(?=\n\s*\n|$)/s);
        if (tableMatch) {
          const tableText = tableMatch[1];
          console.log('Tabela de débitos:', tableText);
          
          // Regex mais flexível para capturar os dados
          // Regex mais precisa para capturar os dados da tabela SIEF
          const regex = /(\d{4}-\d{2}\s*-\s*[A-Z]+(?:\s*-\s*[A-Z]+)?)\s+(\d{2}\/\d{4})\s+(\d{2}\/\d{2}\/\d{4})\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\d+(?:\.\d{3})*(?:,\d{2})?)\s+(\w+)(?:\s+CNO:\s*(\d+(?:\.\d+)*\/\d+))?/g;
          const matches = text.matchAll(regex);
          let count = 0;
          for (const match of matches) {
