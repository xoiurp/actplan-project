...
export async function createOrder(order: any) {
  ...
  if (order.itens_pedido?.length > 0) {
    ...
    const orderItems = order.itens_pedido.map((item: any, index: number) => {
      const formattedItem = {
        order_id: data.id,
        code: item.code,
        tax_type: item.tax_type, // Corrigido: era taxType, agora tax_type
        start_period: formatDateForDB(item.start_period), // Corrigido: era startPeriod
        end_period: formatDateForDB(item.end_period), // Corrigido: era endPeriod
        due_date: formatDateForDB(item.due_date), // Corrigido: era dueDate
        original_value: item.original_value, // Corrigido: era originalValue
        current_balance: item.current_balance, // Corrigido: era currentBalance
        fine: item.fine || 0,
      ...
      // Log detalhado para identificar itens com problemas
      if (!formattedItem.start_period) {
        console.error(`❌ Item ${index} tem start_period null:`, {
          original: item,
    ...
    // Verifica se há itens com campos null que são obrigatórios
    const itemsWithNullFields = orderItems.filter(item => !item.start_period || !item.due_date);
    if (itemsWithNullFields.length > 0) {
      console.error('❌ Itens com campos obrigatórios null:', itemsWithNullFields);
      throw new Error(`${itemsWithNullFields.length} itens têm campos obrigatórios null (start_period ou due_date)`);
    }
