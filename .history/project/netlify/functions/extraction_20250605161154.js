...
function extractPendenciasDebito(text) {
  ...
  while (i < lines.length) {
    ...
    // Check if line starts with a receipt code and contains all data in same line
    const fullLineMatch = line.match(/^(\d{4}-\d{2}\s+-\s+[^0-9]+)\s+(\d{2}\/\d{4}|\d+\s*(?:º|o|ª)?\s*TRIM\/\d{4})\s+(\d{2}\/\d{2}\/\d{4})\s+([\d.,]+)\s+([\d.,]+)\s+(\w+.*?)(?:\s+CNO:\s*([\d.\/-]+))?$/);
    
    if (fullLineMatch) {
      console.log(`\nFull line debit record found: '${line}'`);
      const debitoData = {
        cnpj: currentCnpj,
        receita: fullLineMatch[1].trim(),
        periodo_apuracao: formatPeriodo(fullLineMatch[2].trim()),
        vencimento: formatDate(fullLineMatch[3].trim()),
        valor_original: parseBrCurrency(fullLineMatch[4].trim()),
        saldo_devedor: parseBrCurrency(fullLineMatch[5].trim()),
        multa: 0.0,
        juros: 0.0,
        saldo_devedor_consolidado: 0.0,
        situacao: fullLineMatch[6].trim(),
        cno: fullLineMatch[7] ? fullLineMatch[7].trim() : ""
      };
    ...
    // Try original multi-line format
...
function extractDebitosExigSuspensaSief(text) {
  ...
  while (i < lines.length) {
    ...
    // Check if line starts with a receipt code and contains all data in same line
    const fullLineMatch = line.match(/^(\d{4}-\d{2}\s+-\s+[^0-9]+)\s+(\d{2}\/\d{4}|\d+\s*(?:º|o|ª)?\s*TRIM\/\d{4})\s+(\d{2}\/\d{2}\/\d{4})\s+([\d.,]+)\s+([\d.,]+)\s+(\w+.*?)$/);
...
function extractPendenciasInscricaoSida(text) {
  ...
  while (i < lines.length) {
    ...
    // Check for inscription number with full data in same line
    const fullLineMatch = line.match(/^(\d{2}\.\d{1,2}\.\d{2}\.\d{6}-\d{2})\s+(\d{4}-[^0-9]+)\s+(\d{2}\/\d{2}\/\d{4})\s*(\d{2}\/\d{2}\/\d{4})?\s*([\d.-]+)\s+(\w+.*?)$/);
    
    if (fullLineMatch) {
      console.log(`\nFull line SIDA record found: '${line}'`);
      
      // Extract the next line for situation
      let situacao = "";
      if (i + 1 < lines.length && lines[i + 1].trim().startsWith("Situação:")) {
        situacao = lines[i + 1].trim().replace("Situação:", "").trim();
        i++; // Skip the situation line
      }
      
      const inscricaoData = {
        cnpj: currentCnpj,
        inscricao: fullLineMatch[1],
        receita: fullLineMatch[2],
        inscrito_em: formatDate(fullLineMatch[3]),
        ajuizado_em: fullLineMatch[4] ? formatDate(fullLineMatch[4]) : "",
        processo: fullLineMatch[5],
        tipo_devedor: fullLineMatch[6],
        situacao: situacao
      };
      
      if (inscricaoData.inscricao && inscricaoData.receita) {
...
function extractPendenciasParcelamentoSispar(text) {
  ...
  while (i < lines.length) {
    ...
    // Check for parcelamento with full data in same line (Conta format)
    const fullLineMatch = line.match(/^(\d+)\s+(.+?)$/);
    
    if (fullLineMatch && currentCnpj) {
      console.log(`\nFull line SISPAR record found: '${line}'`);
      
      // Extract modalidade from next line if it exists
      let modalidade = "";
      if (i + 1 < lines.length && lines[i + 1].trim().startsWith("Modalidade:")) {
        modalidade = lines[i + 1].trim().replace("Modalidade:", "").trim();
        i++; // Skip the modalidade line
      }
      
      const parcelamentoData = {
        cnpj: currentCnpj,
        conta: fullLineMatch[1],
        descricao: fullLineMatch[2].trim(),
        modalidade: modalidade
      };
      
      if (parcelamentoData.conta) {
        result.push(parcelamentoData);
